...
class UtilisateurC extends Utilisateur {
    ...
    public function tconnec() 
    {
        ...
        if (empty($_POST["mdp_utilisateur"])) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir votre mot de passe.";
        } else {
            $mdp_utilisateur = test_input($_POST["mdp_utilisateur"]);
        }
        if (empty($_POST["email_utilisateur"])) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir votre email.";
        } else {
            $email_utilisateur = test_input($_POST["email_utilisateur"]);
            if (!filter_var($email_utilisateur, FILTER_VALIDATE_EMAIL)) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Le format de l'email invalide.";
            }
        }
                ...
                if ( $user["email_{$this->_table}"] === $email_utilisateur ) {
                    ...
                    if ( password_verify($mdp_utilisateur, $user["mdp_{$this->_table}"]) ) {
                        if ( !empty($user["confirmat_{$this->_table}"]) ) {
                            ...
                            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Vous n'avez pas validé votre adresse email.";
                        }
                    } else {
                        $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Votre mot de passe est incorrect.";
                    }
                }
            }
            if ($error) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>L'email indiqué n'existe pas.";
            }
        }
        if ( empty($_SESSION['flash']) ) {
            ...
            // Redirection
            $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Vous êtes connecté. Nous vous souhaitons une bonne journée !";
    ...
    public function tinscrip() 
    {
        ...
        // mot de passe
        if ( empty($_POST["mdp_utilisateur"]) ) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir un mot de passe.";
        } else {
            $mdp_utilisateur_tp = test_input($_POST["mdp_utilisateur"]);
            if ( !preg_match('#^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,}$#', $mdp_utilisateur_tp) ) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Format du mot de passe invalide.<br>8 caractères minimum avec au minimum 1 majuscule, 1 minuscule et 1 chiffre.";
            } else {
        ...
        // email
        if (empty( $_POST["email_utilisateur"]) ) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir votre adresse email.";
        } else {
            $email_utilisateur = test_input($_POST["email_utilisateur"]);
            if (!filter_var($email_utilisateur, FILTER_VALIDATE_EMAIL)) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Le format de l'adresse email est invalide.";
            } else {
                ...
                if ( !empty($email_utilisateur_tp) ) {
                    $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Vous êtes déja inscrit. Connectez-vous : <a href='index.php?controller=utilisateur&task=connec'>ICI</a>";
                }
            }
        }
        // prenom
        if (empty($_POST["prenom_utilisateur"])) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir votre prenom.";
        } else {
            ...
            if ( !preg_match('#^[\p{Latin}\' -]+$#u', $prenom_utilisateur) ) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Format du prenom invalide.<br>Seul les caractères alphanumérique et les accents sont autorisés.";
            }
        }
        // nom
        if (empty($_POST["nom_utilisateur"])) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir votre nom.";
        } else {
        ...
        if ( empty($_SESSION['flash']) AND !empty($nom_utilisateur) AND !empty($prenom_utilisateur) AND !empty($email_utilisateur) AND !empty($mdp_utilisateur) ) {
            ...
            if ( empty($utilisateur["id_{$this->_table}"]) ) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Une erreur c'est produite lors de l'insertion dans la base de donnée. Veuillez contacter un administrateur.";
            }
        }
        ...
        if ( empty($_SESSION['flash']) ) {
            ...
            // Redirection
            $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Inscription réussi ! Veuilliez valider votre compte par mail.";
            header('location:index.php?controller=utilisateur&task=connec');
        } else {
    ...
    public function toublie() 
    {
        ...
        if (empty( $_POST["email_utilisateur"]) ) {
            ...
            if (!filter_var($email_utilisateur, FILTER_VALIDATE_EMAIL)) {
                ...
                if ( empty( $user["email_{$this->_table}"] ) ) {
                    $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Nous n'avons pas trouvé l'adresse email.";
                }
            }
        }
        ...
        if ( empty($_SESSION['flash']) AND !empty($email_utilisateur) ) {
            ...
            // Redirection
            $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Email envoyé ! Veuilliez valider votre nouveau mot de passe.";
    ...
    public function mdp()
    {
        ...
        {
            $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Vous ne pouvez pas accèder à cette page.";
            header('location:index.php');
            exit();
        }
    ...
    public function mdpt()
    {
        ...
        if ( empty($_POST["mdp_utilisateur"]) || empty($_POST["md2_utilisateur"]) ) {
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir tous les champs.";
        } else {
            if ( $_POST["mdp_utilisateur"] != $_POST["md2_utilisateur"] ) {
                $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Veuillez saisir les mêmes mots de passe.";
            } else {
                $mdp_utilisateur_tp = test_input($_POST["mdp_utilisateur"]);
                if ( !preg_match('#^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,}$#', $mdp_utilisateur_tp) ) {
                    $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Format du mot de passe invalide.<br>8 caractères minimum avec au minimum 1 majuscule, 1 minuscule et 1 chiffre.";
        ...
        if ( empty($_SESSION['flash']) AND !empty($mdp_utilisateur) ) {
            ...
            // Redirection
            $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Votre mot de passe à bien été réinitialisé.";
            header('location:index.php?controller=utilisateur&task=connec');
            exit();
    ...
    public function validt()
    {
            ...
            $user["confirmat_{$this->_table}"]  == NULL ) 
        {
                ...
                !empty( $user["confirmat_{$this->_table}"] )) 
            {
                ...
                // Redirection
                $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Votre compte à bien été validé.";
                header('location:index.php');
                exit();
            }
            ...
            if ( !empty($_SESSION["id_utilisateur"]) ) {
                $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Votre adresse email a déjà été validé.";
                header('location:index.php');
            } else {
                $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Votre adresse email a déjà été validé. Vous pouvez maintenant vous connecter.";
                header('location:index.php?controller=utilisateur&task=connec');
            }
            exit();
        } else {
            // Redirection
            $_SESSION['flash']['danger'] = "<i class='far fa-bell mr-3'></i>Le lien de validation est incorrecte.";
            header('location:index.php?controller=utilisateur&task=connec');
            exit();
        }
    ...
    public function redirectIfConnec()
    {
        if ( !empty($_SESSION["id_utilisateur"]) ) 
        {
            $_SESSION['flash']['success'] = "<i class='far fa-bell mr-3'></i>Vous êtes déjà connecté.";
            header('location:index.php');
            exit();
        }
    }
